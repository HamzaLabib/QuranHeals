...
// The pre-write backup-destination assertion previously lived here, but this
// whole file fails to initialize whenever backend/data/quran/quran-data.xml
// is absent (loadCorpus() above runs at module scope), so that assertion
// never actually executed. It now lives in
// backend/tests/quran-data/backup-paths.test.ts, which imports only the
// side-effect-free backend/src/utils/backupPaths.ts and therefore runs
// (and passes) even when this file's own suite cannot initialize.
