...
describe('Phase 5B Batch 4 candidate artifact', () => {
  ...
  it('excludes every pair that already received a direct human decision (Batch 1-3 core + supplemental)', () => {
    const batch1 = loadJson<HumanReviewFile>('phase5b-batch1-human-review.json');
    const batch2 = loadJson<HumanReviewFile>('phase5b-batch2-human-review.json');
    const batch3 = loadJson<HumanReviewFile>('phase5b-batch3-human-review.json');
  ...
  it('never stores Quran Arabic or a full translation, and carries no review/status field', () => {
    const raw = readFileSync(resolve(DATA_DIR, 'phase5b-batch4-candidates.json'), 'utf-8');
