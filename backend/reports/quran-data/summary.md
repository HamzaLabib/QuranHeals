...
All 16 existing Arabic checksums reproduce their stored text. Ten match the incoming Arabic exactly; six differ. Eleven existing translations match; five differ. **Import is not safe to proceed under the required conflict-stop rule.** Details: [preflight](preflight.json), [source validation](source-validation.json).
...
The original foundation records are saved in [baseline.json](baseline.json).
...
Exact JSON output: [database validation](database-validation.json). Commands query actual collections, not hardcoded success. No connection credentials are printed. The optional transaction write path is implemented but **not exercised**, so successful import/re-run idempotency is not claimed as tested.
...
This verifies the current database, not a completed full-corpus database. [Exact runtime results and actual indexes](api-regression.json).
