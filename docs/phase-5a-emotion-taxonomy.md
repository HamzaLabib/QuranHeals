# Phase 5A — Emotion taxonomy, aliases, and the candidate-mapping workflow
...
| Emotion–ayah mappings | **43** | sum of `emotions[]` across 16 ayahs in `backend/src/seed/ayahs.ts`; asserted in `backend/tests/fullQuran.test.ts` (`total: 43`, `statuses: { development: 43 }`) |
| Distinct mapped verses | **16** | `backend/tests/fullQuran.test.ts` (`.size).toBe(16)`) |
...
| --- | --- | --- | --- |
| 1 | `backend/src/models/Emotion.ts`, `backend/src/models/Ayah.ts`, `backend/src/models/EmotionVerseMapping.ts`, `backend/src/validators/ayahValidators.ts` (and the copy in `backend/tests/seedIntegrity.test.ts`, plus `LIVE_EMOTION_KEY_PATTERN` in the taxonomy module) | `emotionKey` matched `/^[a-z][a-z-]{1,40}$/` — **letters and hyphens only** | **DONE in 5B prep** — widened to `/^[a-z][a-z_-]{1,40}$/` (letters, hyphens, underscores only). Additive; every existing key still matches. |
| 2 | `backend/tests/seedIntegrity.test.ts` (`maps every active emotion to at least one ayah`) | every `active` emotion has ≥1 seed mapping | **Satisfied** — new emotions are seeded `active: false`, so this check still passes. A key flips to `active: true` only once it has ≥1 approved mapping. |
...
| Review-friendly output | `reports/phase5a-candidate-mappings.json` + `.md` (summary table, issue counts, per-row status). |
...
unit-tested in `backend/tests/candidateMappingImport.test.ts`.
...
- `backend/tests/fullQuran.test.ts` still asserts `total: 43`,
...
`backend/tests/seedIntegrity.test.ts`, and `LIVE_EMOTION_KEY_PATTERN` /
